...
const TrackOrderPage = () => {
  ...
  const userAvatar =
    profile?.avatar_url ||
    `https://api.dicebear.com/7.x/initials/svg?seed=${userFirstName[0]}${userLastName[0]}`;
  ...
  // ✅ Abre rastreamento em tempo real SEM pedir endereço
  ...
  const handleReorder = async (orderId: string) => {
    toast.info("Função de repetir pedido em desenvolvimento.");
  };
    ...
    <div className="container py-8 grid grid-cols-1 lg:grid-cols-[250px_1fr] gap-8">
      ...
      <aside className="space-y-6">
        ...
        <nav className="grid gap-2 text-sm font-medium">
          ...
          <Button
            variant="ghost"
            className="flex items-center justify-start gap-3 rounded-lg px-3 py-2 text-destructive hover:text-destructive hover:bg-destructive/10"
            ...
            {currentOrder ? (
              <>
                <div className="flex items-center justify-between">
                  <p className="font-medium">Pedido #{currentOrder.id.slice(0, 8)}</p>
                  <Badge variant={getStatusBadgeVariant(currentOrder.status)}>
                    {currentOrder.status}
                  </Badge>
                </div>
                <p className="text-sm text-muted-foreground">
                  Entrega estimada: {currentOrder.estimateddeliverytime || "N/A"}
                </p>
                <p className="text-sm text-muted-foreground">
                  Itens: {currentOrder.items}
                </p>
                ...
                {/* ✅ BOTÃO DE RASTREIO EM TEMPO REAL */}
                <div className="mt-4">
                  <Button
                    className="w-full"
