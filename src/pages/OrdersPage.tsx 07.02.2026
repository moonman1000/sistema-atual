...
const OrdersPage = () => {
  ...
  // não usamos diretamente availableDeliverymenNames do contexto aqui; buscaremos localmente
  ...
  // novo estado: entregadores como objetos e array de nomes para compatibilidade
  const [availableDeliverymen, setAvailableDeliverymen] = useState<DeliverymanObj[]>([]);
  const [availableDeliverymenNames, setAvailableDeliverymenNames] = useState<string[]>([]);
  ...
  // BUSCA os entregadores (employees) como {id, name}
  useEffect(() => {
  ...
  const handleEditOrder = async (updatedOrder: Order) => {
    console.log("DEBUG: deliveryman antes de atualizar:", updatedOrder.deliveryman);

    try {
      // Atualiza o pedido
      const result = await updateOrder(updatedOrder);
      const finalOrder = result || updatedOrder;

      markOrderAsViewed(finalOrder.id);
      toast.success("Pedido atualizado com sucesso!");

      // Sincroniza com deliveries
      if (["Em Entrega", "Problema", "Entregue", "Recusado", "Devolvido"].includes(finalOrder.status)) {
        const existingDelivery = deliveries.find(d => d.orderid === finalOrder.id);
        let deliveryStatus: Delivery["status"];
        let actualDeliveryTime: string | undefined = undefined;

        if (finalOrder.status === "Em Entrega") deliveryStatus = "Em Entrega";
        else if (finalOrder.status === "Problema") deliveryStatus = "Problema";
        else if (finalOrder.status === "Recusado") deliveryStatus = "Recusado";
        else if (finalOrder.status === "Devolvido") deliveryStatus = "Devolvido";
        else {
          deliveryStatus = "Entregue";
          actualDeliveryTime = new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });
        }

        // TENTAR mapear deliveryman name para driver_profile_id
        const deliverymanName = finalOrder.deliveryman || "";
        const matched = availableDeliverymen.find(d => d.name === deliverymanName);
        const driver_profile_id = matched ? matched.id : undefined;
        if (!driver_profile_id) {
          console.warn("Entrega: não foi possível mapear deliveryman para driver_profile_id:", deliverymanName);
        }

        if (!existingDelivery) {
          console.log("📦 Criando nova entrega para o entregador:", deliverymanName, "uid:", driver_profile_id);
          await addDelivery({
            orderid: finalOrder.id,
            clientname: finalOrder.client_name,
            client_address: finalOrder.client_address,
            deliveryman: deliverymanName,
            driver_profile_id, // pode ser undefined se não mapeado
            status: deliveryStatus,
            estimateddeliverytime: "A definir",
            actualdeliverytime: actualDeliveryTime,
            trackinglink: finalOrder.tracking_link,
          } as any);
        } else {
          console.log("🔄 Atualizando entrega existente para o entregador:", deliverymanName, "uid:", driver_profile_id);
          await updateDelivery({
            ...existingDelivery,
            deliveryman: deliverymanName,
            driver_profile_id: driver_profile_id ?? existingDelivery.driver_profile_id,
            clientname: finalOrder.client_name,
            client_address: finalOrder.client_address,
            status: deliveryStatus,
            trackinglink: finalOrder.tracking_link,
            actualdeliverytime: actualDeliveryTime || existingDelivery.actualdeliverytime,
          } as any);
        }
      }
    } catch (error: any) {
  ...
  const statusOptions = ["Todos", "Pendente", "Confirmado", "Em Preparo", "Em Entrega", "Entregue", "Cancelado", "Problema", "Recusado", "Devolvido"];
  ...
  return (
    <div className="space-y-6">
            ...
            {filteredOrders.map((order: any) => {
              ...
              return (
                <TableRow
                  ...
                  className={cn(
                    "cursor-pointer",
                    isNew && "bg-yellow-50/50 dark:bg-yellow-900/20 animate-pulse-new-order"
                  )}
      ...
      <AddOrderDialog 
        ...
        availableDeliverymen={availableDeliverymenNames} // compatibilidade
      />
      <ViewOrderDetailsDialog 
        isOpen={isViewDetailsDialogOpen} 
        onClose={() => setIsViewDetailsDialogOpen(false)} 
        order={viewingOrder} 
        markOrderAsViewed={markOrderAsViewed} 
      />
      {editingOrder && (
        <EditOrderDialog 
          ...
          availableClients={availableClients} 
          availableDeliverymen={availableDeliverymenNames} 
          markOrderAsViewed={markOrderAsViewed} 
        />
      )}
    </div>
