...
const OrdersPage = () => {
  ...
  const handleEditOrder = async (updatedOrder: Order) => {
    try {
      ...
      if (updatedOrder.status === "Em Entrega" || updatedOrder.status === "Problema") {
        const existingDelivery = deliveries.find(d => d.orderid === updatedOrder.id);
        const deliveryStatus = updatedOrder.status === "Em Entrega" ? "Em Entrega" : "Problema";
        if (!existingDelivery) {
          await addDelivery({
            ...
            status: deliveryStatus as Delivery["status"],
            estimateddeliverytime: "A definir",
            trackinglink: updatedOrder.tracking_link,
          });
        } else if (existingDelivery.status !== deliveryStatus) {
          await updateDelivery({ ...existingDelivery, status: deliveryStatus as Delivery["status"], trackinglink: updatedOrder.tracking_link });
        }
      }
    } catch (error: any) {
  ...
  const handleDeleteOrder = async (orderId: string) => {
    try {
      ...
      markOrderAsViewed(orderId);
      toast.success(`Pedido ${orderId} foi excluÃ­do.`);
    } catch (error: any) {
  ...
  const handleGenerateInvoice = (order: Order) => {
    if (!currentRestaurant) {
      toast.error("NÃ£o foi possÃ­vel gerar a nota fiscal: dados do restaurante ausentes.");
      return;
    }
    try {
  ...
  const statusOptions = ["Todos", "Pendente", "Confirmado", "Em Preparo", "Em Entrega", "Entregue", "Cancelado", "Problema"];
  ...
  if (!Array.isArray(orders)) {
    return (
      <div className="p-6">
        ...
        <p>O estado de pedidos nÃ£o Ã© um array. Veja o console para detalhes.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      {process.env.NODE_ENV !== "production" && (
        <div className="p-3 bg-yellow-50 border border-yellow-200 rounded text-sm">
          <strong>DEBUG â€” OrdersPage</strong>
      ...
      <AdminTableFiltersWrapper>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={() => setSortBy("daily_number-desc")}>NÂº DiÃ¡rio (Recente)</DropdownMenuItem>
            <DropdownMenuItem onClick={() => setSortBy("daily_number-asc")}>NÂº DiÃ¡rio (Antigo)</DropdownMenuItem>
      ...
      <div className="border rounded-lg overflow-hidden">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead className="w-[100px]">NÂº DiÃ¡rio</TableHead>
            ...
            {filteredOrders.map((order: any) => {
              ...
              return (
                <TableRow
                  ...
                  className={cn(
                    "cursor-pointer",
                    isNew && "bg-yellow-50/50 dark:bg-yellow-900/20 border-l-4 border-orange-500 animate-pulse-new-order"
                  )}
