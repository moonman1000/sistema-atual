...
import { Loader2, ChevronDown, MapPin, Search as SearchIcon, ShoppingCart } from "lucide-react"; // Usando o ícone ShoppingCart
...
import MarketCard from "@/components/sales/MarketCard"; // Importar o novo componente MarketCard
...
const MercadosPage = () => {
  ...
  const filteredAndSortedMarkets = useMemo(() => {
    let currentMarkets = allRestaurants;

    // Filtrar para mostrar apenas estabelecimentos do tipo 'market'
    currentMarkets = currentMarkets.filter(r => r.type === 'market');

    if (searchTerm) {
      currentMarkets = currentMarkets.filter(
        (market) =>
    ...
    if (addressSearchTerm) {
      currentMarkets = currentMarkets.filter(
        (market) =>
          market.address?.toLowerCase().includes(addressSearchTerm.toLowerCase())
      );
    }
  ...
  // Filter promotions to only include those from 'market' type establishments
  const marketTypePromotions = useMemo(() => {
    const marketIds = allRestaurants.filter(r => r.type === 'market').map(r => r.id);
    return allGlobalPromotions.filter(promo => marketIds.includes(promo.restaurant_id));
  ...
  if (isLoadingRestaurants || isLoadingGlobalPromotions || isLoadingGlobalMenuItems) {
    return (
      <div className="flex min-h-screen items-center justify-center">
        ...
        <p className="ml-2">Carregando mercados e promoções...</p>
      </div>
    );
  }
  ...
  return (
    <div className="container py-12">
      <div className="text-center mb-12">
        <h1 className="text-5xl font-extrabold mb-4 text-primary">Descubra Nossos Mercados</h1>
        <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
          Encontre produtos frescos, itens essenciais e muito mais perto de você.
        </p>
      </div>
      ...
      <Card className="mb-12 p-6 shadow-lg">
        <CardHeader className="pb-4">
          <CardTitle className="text-2xl font-bold">Encontre seu Mercado</CardTitle>
      ...
      <PromotionsCarousel promotions={marketTypePromotions} menuItems={allGlobalMenuItems} />
      ...
      {filteredAndSortedMarkets.length > 0 ? (
        ...
        <div className="text-center py-12 border rounded-lg bg-muted/50 mt-12">
          <h3 className="text-2xl font-semibold mb-2 text-primary">Nenhum mercado encontrado.</h3>
          <p className="text-lg text-muted-foreground">
            Ajuste sua busca ou verifique se há mercados cadastrados.
          </p>
        </div>
      )}
    </div>
