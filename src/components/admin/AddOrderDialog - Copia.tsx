...
import { useMenu, MenuItem } from "@/context/MenuContext"; // Importar useMenu e MenuItem
import { PlusCircle, MinusCircle, Loader2 } from "lucide-react"; // Importar ícones
import { toast } from "sonner";
import { formatCurrency } from "@/lib/utils";

// Definir uma interface para os itens do pedido no formulário
...
interface AddOrderDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onAddOrder: (newOrder: Omit<Order, 'id' | 'profile_id' | 'created_at' | 'updated_at'>, orderItems: FormOrderItem[]) => void;
  availableClients: string[];
}

type OrderStatus = "Confirmado" | "Em Preparo" | "Em Entrega" | "Entregue" | "Cancelado" | "Problema" | "Pendente" | "Recusado" | "Devolvido"; // ADICIONADO: Recusado, Devolvido

const AddOrderDialog: React.FC<AddOrderDialogProps> = ({ isOpen, onClose, onAddOrder, availableClients }) => {
  const { menuItems, isLoadingMenuItems } = useMenu(); // Obter itens do cardápio
  const [clientName, setClientName] = React.useState("");
  const [client_address, setClientAddress] = React.useState("");
  const [total, setTotal] = React.useState("");
  const [status, setStatus] = React.useState<OrderStatus>("Pendente"); // ALTERADO: Default para Pendente
  ...
  const [formOrderItems, setFormOrderItems] = React.useState<FormOrderItem[]>([]); // Estado para os itens do pedido
  const [isAdding, setIsAdding] = React.useState(false);

  // Calcula o total dos itens do pedido dinamicamente
  ...
  // Atualiza o campo 'total' do formulário com base no cálculo dos itens
  React.useEffect(() => {
  ...
  const handleAddItem = () => {
    setFormOrderItems(prev => [
      ...prev,
      {
        tempId: Math.random().toString(36).substring(2, 11), // ID temporário para a UI
        menuItemId: "",
        name: "",
        quantity: 1,
        basePrice: 0,
      ...
      prev.map(item => {
        if (item.tempId === tempId) {
          if (field === "menuItemId") {
            ...
            if (selectedMenuItem) {
              // Resetar tamanhos e coberturas ao mudar o item
            ...
            // value aqui é o ID da cobertura que foi marcada/desmarcada
            const selectedMenuItem = menuItems.find(mi => mi.id === item.menuItemId);
      ...
      await onAddOrder(
        {
          ...
          client_address,
          items: orderItemsDescription, // Descrição textual dos itens
        ...
        formOrderItems // Passar os itens estruturados
      );
      onClose();
      // Reset form fields
      setClientName("");
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Select value={status} onValueChange={(value: OrderStatus) => setStatus(value)}>
              ...
              <SelectContent>
                <SelectItem value="Pendente">Pendente</SelectItem> {/* NOVO: Adicionado Pendente */}
