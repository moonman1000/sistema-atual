...
interface AddProductDialogProps {
  ...
  onClose: () => void;
  onAddProduct: (
    newProduct: Omit<MenuItem, 'id' | 'created_at' | 'updated_at' | 'sizes' | 'toppings' | 'reviews'>,
...
const AddProductDialog: React.FC<AddProductDialogProps> = ({ isOpen, onClose, onAddProduct, categories, restaurantType }) => {
  ...
  const [sizes, setSizes] = React.useState<Omit<MenuItemSize, 'id' | 'menu_item_id' | 'created_at' | 'updated_at'>[]>(defaultPizzaSizes);
  const [toppings, setToppings] = React.useState<Omit<MenuItemTopping, 'id' | 'menu_item_id' | 'created_at' | 'updated_at'>[]>([]);
  ...
  // Effect to initialize category and isSizeEnabled when dialog opens or categories change
  React.useEffect(() => {
    if (isOpen) {
      if (categories.length > 0 && !category) {
        setCategory(categories[0]);
      }
      const normalizedCategory = normalizeCategoryName(category || (categories.length > 0 ? categories[0] : ""));
      const isBeverageOrHamburger = normalizedCategory === "bebida" || normalizedCategory === "hamburger";
      setIsSizeEnabled(!isBeverageOrHamburger); // Suggest initial state based on category
    }
  }, [isOpen, categories, category, normalizeCategoryName]);

  // Effect to manage sizes array based on isSizeEnabled state
  React.useEffect(() => {
    if (!isSizeEnabled) {
      setSizes([]);
    } else if (sizes.length === 0) {
      setSizes(defaultPizzaSizes);
    }
  }, [isSizeEnabled, sizes.length]);
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    console.log("[DEBUG] AddProductDialog: handleFileChange triggered.");
  ...
  const addSize = () => {
    setSizes(prev => [...prev, { name: "", value: "", price_modifier: 0 }]);
  };
  ...
  const addTopping = () => {
    setToppings(prev => [...prev, { name: "", value: "", price: 0 }]);
  };
  ...
  const handleCloseDialog = () => {
    ...
    setIsFeatured(false);
    setDietary([]);
    setIsSizeEnabled(true); // Reset to default enabled
    setSizes(defaultPizzaSizes); 
    setToppings([]);
  ...
  const handleSubmit = async () => {
    ...
    if (isSizeEnabled && sizes.length === 0) {
    ...
    try {
      if (imageFile) {
        ...
        if (uploadedUrl) {
          ...
          toast.error("Falha ao obter URL da imagem após o upload.");
          setIsUploading(false);
          return;
        }
      }

      const newProductData: Omit<MenuItem, 'id' | 'created_at' | 'updated_at' | 'sizes' | 'toppings' | 'reviews'> = {
        name,
        description,
      ...
      const sizesToSave = isSizeEnabled ? sizes : [];

      onAddProduct(newProductData, sizesToSave, toppings);
      handleCloseDialog();
    } catch (error: any) {
    ...
    <Dialog open={isOpen} onOpenChange={handleCloseDialog}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Preencha os detalhes para adicionar um novo item ao cardápio.
          </DialogDescription>
        </DialogHeader>
