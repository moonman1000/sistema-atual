...
interface AddExpenseDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onAddExpense: (newExpense: Omit<Expense, 'id' | 'restaurant_id' | 'created_at' | 'updated_at'>) => Promise<void>;
...
const AddExpenseDialog: React.FC<AddExpenseDialogProps> = ({ isOpen, onClose, onAddExpense, availableSuppliers, availableEmployees }) => {
  const [type, setType] = React.useState<Expense['type']>("Conta Fixa");
  const [description, setDescription] = React.useState("");
  const [amount, setAmount] = React.useState("");
  const [date, setDate] = React.useState<Date | undefined>(createLocalDate(new Date().toISOString().split('T')[0])); // Usar createLocalDate
  const [status, setStatus] = React.useState<Expense['status']>("Pendente");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setDescription("");
      setAmount("");
      setDate(createLocalDate(new Date().toISOString().split('T')[0])); // Usar createLocalDate
      setStatus("Pendente");
      setRelatedEntity("");
    } catch (error) {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Registre um novo custo operacional.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button type="submit" onClick={handleSubmit} disabled={isAdding}>
            {isAdding ? "Adicionando..." : "Salvar Despesa"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
